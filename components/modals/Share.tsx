...
export default function ShareModal({ link, message, closeCallback, noBottomPadding }: Params): JSX.Element {
    ...
    return <PanGestureHandler onGestureEvent={handleGesture} onHandlerStateChange={e => handleGestureEnd(e)} >
        <Animated.View style={{
            ...
            bottom: 0, height: 200 + (noBottomPadding ? 0 : safeArea.bottom), backgroundColor: 'white', paddingHorizontal: 16,
            transform: [
                {
            ...
            <Button style={{ height: 56, borderRadius: 0, backgroundColor: Theme.colors.background }} block onPress={() => { Clipboard.setString(link); setCopyLinkText('Copied') }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'row', marginTop: 16 }} >
                <Button style={{ flexGrow: 1, height: 56, borderRadius: 0, borderWidth: 3, borderColor: Theme.colors.background, backgroundColor: 'transparent', marginRight: 16 }} block onPress={shareToTwitter}>
                    <Thumbnail square source={Theme.icons.black.twitter} style={{ width: 24, height: 24 }} ></Thumbnail>
                </Button>
                <Button style={{ flexGrow: 1, height: 56, borderRadius: 0, borderWidth: 3, borderColor: Theme.colors.background, backgroundColor: 'transparent' }} block onPress={share}>
